...
func TestChannelStoreUpdateLastViewedAt(t *testing.T) {
	...
	if result := <-store.Channel().UpdateLastViewedAt([]string{m1.ChannelId}, m1.UserId); result.Err != nil {
	...
	if result := <-store.Channel().UpdateLastViewedAt([]string{m1.ChannelId, m2.ChannelId}, m1.UserId); result.Err != nil {
		t.Fatal("failed to update", result.Err)
	} else if result.Data.(map[string]int64)[o2.Id] != o2.LastPostAt {
		t.Fatal("last viewed at time incorrect")
	}

	rm1 := Must(store.Channel().GetMember(m1.ChannelId, m1.UserId)).(*model.ChannelMember)
	...
	rm2 := Must(store.Channel().GetMember(m2.ChannelId, m2.UserId)).(*model.ChannelMember)
	...
	if result := <-store.Channel().UpdateLastViewedAt([]string{m1.ChannelId}, "missing id"); result.Err != nil {
