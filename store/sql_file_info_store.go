...
func (fs SqlFileInfoStore) AttachToPost(fileId, postId string) StoreChannel {
	...
	go func() {
		...
		if _, err := fs.GetMaster().Exec(
			...
			result.Err = model.NewAppError("SqlFileInfoStore.AttachToPost", "store.sql_file_info.attach_to_post.app_error", nil, "post_id="+postId+", file_id="+fileId+", err="+err.Error(), http.StatusInternalServerError)
		}
...
func (fs SqlFileInfoStore) DeleteForPost(postId string) StoreChannel {
	...
	go func() {
		...
		if _, err := fs.GetMaster().Exec(
			...
			result.Err = model.NewAppError("SqlFileInfoStore.DeleteForPost", "store.sql_file_info.delete_for_post.app_error", nil, "post_id="+postId+", err="+err.Error(), http.StatusInternalServerError)
		} else {
			result.Data = postId
		}
